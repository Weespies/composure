...
public class EquipmentManager {
	...
	/**
	 * Read armor list from local file.  If local file does not exist, extract from resource.
	...
	private static void loadArmor() throws UnmarshallingException {
		List<Armor> armor = (List<Armor>)LocalFileController.getInstance()
			.unmarshalResource(ARMOR_RESOURCE, ARMOR_MAPPING);
		armorList.clear();
		armorList.addAll(armor);
	}
	
	/**
	 * Read melee weapon list from local file.  If local file does not exist, extract from resource.
	 * @throws UnmarshallingException 
	 * @throws JDOMException
	 * @throws IOException
	 */
	...
	private static void loadMeleeWeapons() throws UnmarshallingException {
		List<MeleeWeapon> melee = (List<MeleeWeapon>)LocalFileController.getInstance().
		unmarshalResource(MELEE_WEAPON_RESOURCE, MELEE_WEAPON_MAPPING);
		meleeList.clear();
		meleeList.addAll( melee );
	}
	
	/**
	 * Read ranged weapon list from local file.  If local file does not exist, extract from resource.
	 * @throws JDOMException
	 * @throws IOException
	 * @throws UnmarshallingException 
	 */
	...
	private static void loadRangedWeapons() throws UnmarshallingException {
		rangedList.clear();
		List<RangedWeapon> unmarshalled = (List<RangedWeapon>)LocalFileController.getInstance().
			unmarshalResource(RANGED_WEAPON_RESOURCE, RANGED_WEAPON_MAPPING);
		rangedList.addAll( unmarshalled );
		
	}
	
	/**
	 * Read explosives list from local file.  If local file does not exist, extract from resource.
	 * @throws JDOMException
	 * @throws IOException
	 * @throws UnmarshallingException 
	 */
	...
	private static void loadExplosives() throws UnmarshallingException {
		List<Explosive> explosives = (List<Explosive>)LocalFileController.getInstance()
			.unmarshalResource(EXPLOSIVES_RESOURCE, EXPLOSIVES_MAPPING);
		explosivesList.clear();
		explosivesList.addAll( explosives );
	}	
	
	/**
	 * Read equipment list from local file.  If local file does not exist, extract from resource.
	 * @throws JDOMException
	 * @throws IOException
	 * @throws UnmarshallingException 
	 */
	...
	private static void loadEquipment() throws UnmarshallingException {
		List<Equipment> equip = (List<Equipment>)LocalFileController.getInstance()
		.unmarshalResource(EQUIPMENT_RESOURCE,EQUIPMENT_MAPPING);
		equipmentList.clear();
	 ...
	 * @param resource name of the data resource.
	 * @return
	 * @throws IOException 
	 * @throws JDOMException 
	 */
