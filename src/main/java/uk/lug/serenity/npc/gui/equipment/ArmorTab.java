 ...
 *
 */
public class ArmorTab extends JPanel implements DataModelListener<Person> {
	private static final long serialVersionUID = 1L;
	private EquipmentSelector<Armor> selector;
	private DataModel<Person> model;
	private MutableList<Armor> equipmentCatalog;
	 ...
	 * @param dataModel
	 */
	public ArmorTab( DataModel<Person> dataModel ) {
		model= dataModel;
		model.addDataModelListener( this );
		equipmentCatalog = new MutableList<Armor>();
		equipmentCatalog.addAll( EquipmentManager.getArmorList() );
		makeGUI();
	...
	private void makeGUI() {
		...
		selector = new EquipmentSelector<Armor>( new MutableList<Armor>() , equipmentCatalog );
	...
	public void dataChanged(Person oldData, Person newData) {
		selector.setSelectedList( newData.getArmor() );
	}
}
